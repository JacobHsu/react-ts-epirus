...
import { ELinkType, IDictionary } from 'models/models-general';
...
export interface ITransactionCount {
  total: IDictionary<number>;
  data: IDictionary<IChartData>;
}

export interface IDashboardFixtures {
  overview: any;
  transactionCount: ITransactionCount;
  mostActiveContracts: any;
  tokens: any;
}
...
export const DASHBOARD_FIXTURES = {
  ...
  mostActiveContracts,
  tokens: {
    labels: ['Fungible (ERC20)', 'Non-Fungible (ERC721)'],
    datasets: [
      {
        label: 'title',
        data: [174282, 82015],
        backgroundColor: ['#C6F1E7', '#F8F2BF'],
      },
    ],
  },
};
