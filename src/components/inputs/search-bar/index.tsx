...
function SearchBar({
  ...
  const dataWithLinks = useMemo(() => {
    if (data) {
      return data.data.map((value) => {
        const [page] = value.link.href.split('/').filter((part) => Boolean(part));
        return {
          ...value,
          link: {
            url: `/${page}/[detailsHash]`,
  ...
  const selectItemWithIndex = useCallback(
    (index: number) => {
      const item = dataWithLinks[index];

      if ('link' in item) {
        const link = item.link;
        router.push(link.url, link.href);
      }
    },
    [dataWithLinks, router],
  );
